<template>
  <div>
    <h3>模块管理</h3>
    <el-button type="primary" icon="el-icon-plus" @click="addfatheredit">新增父模块</el-button>
  ...
  methods: {
    ...
    addchildren () {
      ...
      axios
        ...
        .then((res) => {
          console.log(res.data)
          const newChild = { type: '1', title: '新建子模块', children: [] }
          if (!res.data.children) {
            that.$set(res.data, 'children', [])
          }
          res.data.children.push(newChild)
        })
    },
    edit (node, data) {
      const parent = node.parent
      this.form.menuid = data.title
      this.form.fid = parent.data.menuid
